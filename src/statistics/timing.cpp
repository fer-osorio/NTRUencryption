...
template<typename T> static std::vector<uint32_t> run_test(std::function<T()> func, TestID tc) {
    ...
    for(int i = 0; i < NUMBER_OF_ROUNDS; i++){
        if((i & 65) == 0) std::cout << tc.get_label() << " test: Round "<< i << std::endl;
        begin = std::chrono::steady_clock::now();
        func();//e.setKeys();// arr_ptr_methods[tc.get_ID]()
...
Encryption::encryption_test(const TestID& t_id,
    ...
    std::unique_ptr<NTRU::Encryption> ptr_e;
    switch(t_id.get_ID_num()){
        case TestIDnum::KEY_GENERATION:
            ptr_e = std::make_unique<NTRU::Encryption>(new NTRU::Encryption(e));
